...
class InMemoryKanbanRepository implements KanbanRepository {
  ...
  int _nextColumnaId = 1;

  /// [conDatosDemo] en `false` deja el tablero vacío (solo las columnas
  /// estándar, sin tareas/etiquetas/miembros de ejemplo) — usado por las
  /// áreas de trabajo que crea la persona usuaria, para que no empiecen
  /// todas mostrando el mismo tablero de demostración.
  InMemoryKanbanRepository({bool conDatosDemo = true}) {
    if (conDatosDemo) _seed();
  }
  ...
  Future<int> agregarActividad(
    ...
    _tareas[idx] = _tareas[idx].copyWith(
      actividades: _conTerminadaDerivada(actividades),
    );
  ...
  Future<void> toggleActividad(int tareaId, int actividadId) async {
    ...
    // Solo las hojas (sin subActividades) se marcan a mano — una actividad
    // con hijas funciona como una "lista": su estatus se deriva solo de
    // ellas (ver [_conTerminadaDerivada]), no se pisa a mano.
    if (actual != null && actual.subActividades.isNotEmpty) return;
    ...
    await _recalcularCompletadoPorActividades(tareaId);
    if (actual != null) {
      _registrarHistorial(
        tareaId,
        actual.terminada
  ...
  /// Recalcula, de abajo hacia arriba, el estatus derivado de cada
  /// actividad que tenga subActividades: una actividad "con hijas" funciona
  /// como una lista (p. ej. "Diseño") y su `terminada` no se marca a mano,
  /// se deriva sola de si TODAS sus hijas (ya recalculadas, así que aplica
  /// a cualquier profundidad de delegación) están terminadas. Las hojas
  /// (sin subActividades, las "tareas" reales) conservan su valor tal cual,
  /// porque esas sí las marca la persona con el checkbox.
  List<Actividad> _conTerminadaDerivada(List<Actividad> lista) {
    final resultado = <Actividad>[];
    for (final a in lista) {
      final hijas = _conTerminadaDerivada(a.subActividades);
      if (hijas.isEmpty) {
        resultado.add(a.copyWith(subActividades: hijas));
      } else {
        resultado.add(
          a.copyWith(
            subActividades: hijas,
            terminada: hijas.every((h) => h.terminada),
          ),
        );
      }
    }
  ...
  /// Si TODAS las actividades de nivel raíz de la tarjeta quedan
  /// terminadas (y, por transitividad de [_conTerminadaDerivada], también
  /// todas sus subtareas a cualquier profundidad), la tarjeta se marca
  /// lista sola y se mueve a "Terminado" — el mismo automatismo que
  /// [_recalcularBloqueoPorSubtareas] pero en sentido de "listo" en vez de
  /// "bloqueado". No dispara con la lista de actividades vacía (nada que
  /// completar no es lo mismo que "ya está completo") ni en una tarjeta ya
  /// cerrada o archivada.
  Future<void> _recalcularCompletadoPorActividades(int tareaId) async {
    final idx = _indice(tareaId);
    final t = _tareas[idx];
    ...
    _registrarHistorial(
      tareaId,
      'Se completó y movió a "Terminado" automáticamente: todas sus '
      'actividades quedaron listas',
    );
  }
  ...
  @override
  Future<KanbanColumna> crearColumna(String titulo) async {
    await _latencia();
    final nombre = titulo.trim().isEmpty ? 'Nueva lista' : titulo.trim();
    final estatus = TareaEstatus.personalizado(
      'personalizada_${_nextColumnaId++}',
    );
    final columna = KanbanColumna(
      estatus: estatus,
      titulo: nombre,
